...
<div style="background: linear-gradient(135deg, #0D1117, #161B22, #1a0a0a);
            border: 1px solid #30363D; border-left: 4px solid #E63946;
    ...
    <p style="color: #8B949E; font-size: 0.9rem; margin: 0;">
        Z stage + knife-edge stage + power meter all automated · Hands-free operation
    </p>
</div>
""", unsafe_allow_html=True)
...
missing = []
if not ophir or not ophir.available: missing.append("Ophir StarBright")
if not smc or not smc.available: missing.append("Newport SMC100")
if not kdc or not kdc.available: missing.append("Thorlabs KDC101")
...
st.info(f"Will scan **{int(z_steps)}** Z positions × **{int(x_steps)}** X points = "
...
if st.button("Start Full Auto Scan", type="primary", use_container_width=True):
    ...
    for z_idx, z_target in enumerate(z_positions):
        ...
        if x_16 is not None and x_84 is not None:
            ...
            all_w.append(w_um)
            log_area.markdown(
                f"Z = {z_actual:.3f} mm → **w = {w_um:.1f} µm** "
                f"(x₁₆ = {x_16:.3f}, x₈₄ = {x_84:.3f})")
    ...
    if len(all_z) >= 3:
        ...
        if fit:
            ...
            st.success(f"w₀ = {fit['w0']:.2f} µm  |  M² = {fit['M2']:.2f}  |  "
                       f"z_R = {fit['z_R']:.0f} µm")
